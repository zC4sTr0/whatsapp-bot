...
async function messageWorker(msg: WAWebJS.Message) {
  ...
  if (global_state_user === 0) {
    msg.reply(
      "Olá! Bem-vindo ao sistema de marcação do Barbergram! Deseja agendar um horário? Digite 'sim' ou 'não'.",
    );
  ...
  if (global_state_user === 1) {
    if (message_received.toLowerCase() === "sim") {
      msg.reply(
        "Excelente! Escreva a data que deseja agendar o horário no formato dia/mes. Exemplo: 01/03",
      );
      global_state_user = 2; //aguardando resposta do usuário para agendar horário
    }

    if (
      message_received.toLowerCase() === "não" ||
      message_received.toLowerCase() === "nao"
    ) {
  ...
  if (global_state_user === 2) {
    ...
    if (
      message_received.match(/^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[0-2])$/) ==
      null
    ) {
      msg.reply(
        "Data inválida! Por favor, informe a data no formato dia/mes. Exemplo: 01/03",
      );
      return;
    }

    const dia_splitado = message_received.split("/")[0];
    const mes_splitado = message_received.split("/")[1];

    if (
      Array.isArray(message_received.split("/")) === false ||
      message_received.split("/").length !== 2
    ) {
      msg.reply(
        "Data inválida! Por favor, informe a data no formato dia/mes. Exemplo: 01/03",
      );
    ...
    if (
      message_received.split("/")[0] === "" ||
      message_received.split("/")[1] === ""
    ) {
      msg.reply(
        "Data inválida! Por favor, informe a data no formato dia/mes. Exemplo: 01/03",
      );
    ...
    if (
      message_received.split("/")[0] == undefined ||
      message_received.split("/")[1] === undefined
    ) {
      msg.reply(
        "Data inválida! Por favor, informe a data no formato dia/mes. Exemplo: 01/03",
      );
    ...
    msg.reply(
      "Certo. Você deseja agendar para a data " + dia + "/" + mes + ".",
    );

    msg.reply(
      "Nesta data, temos os seguintes horários disponíveis: 10:00, 11:00, 14:00 e 15:00. Qual destes horários você prefere?",
    );
    msg.reply(
      "Por favor, me informe apenas o horário que deseja agendar no formato HH:MM. Exemplo: 10:00",
    );
    global_state_user = 3;
  }
  ...
  if (global_state_user === 3) {
    ...
    if (message_received.match(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/) == null) {
      msg.reply(
        "Horário inválido! Por favor, informe o horário no formato HH:MM. Exemplo: 10:00",
      );
      return;
    }

    const hora_splitada = message_received.split(":")[0];
    const minuto_splitado = message_received.split(":")[1];
    ...
    msg.reply(
      "Certo. Você deseja agendar para o horário " + hora + ":" + minuto + ".",
    );
  ...
  if (global_state_user === 4) {
    const nome = message_received;
    msg.reply(
      "Agendado! Horário agendado para " +
        nome +
        " no dia " +
        dia +
        "/" +
...
console.log("Variavel de ambiente O ENAI_API_KEY: " + process.env.OPENAI_KEY);
